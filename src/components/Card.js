...
export default function Card({ handleGameCompletion, setSuccess, success }) {
  ...
  return (
    <>
      <Logo />
      <div
        className={border ? `card ${borderColor}` : "card"}
        data-identifier="flashcard"
      >
        <span data-identifier="counter">
          {currentQuestion}/{deck.length}
        </span>

        <h2>{answerActive ? deck[currentQuestion - 1].question : ""}</h2>
        ...
        {footerButtons ? (
          <div className="cardFooter">
            <button
              className="cardButton black"
              onClick={handleClickFooterButtonBlack}
            >
              Aprendi Agora
            </button>
            <button
              className="cardButton red"
              onClick={handleClickFooterButtonRed}
            >
              Não lembrei
            </button>
            <button
              className="cardButton green"
              onClick={handleClickFooterButtonGreen}
            >
              Lembrei com esforço
            </button>
            <button
              className="cardButton yellow"
              onClick={handleClickFooterButtonYellow}
            >
              Zap!
            </button>
          </div>
        ) : turnAnswer ? (
          <ArrowIcon handleClick={handleClickTurnAnswer} />
        ) : (
          <ArrowIcon handleClick={handleClickTurnNextCard} />
        )}
      </div>
    </>
  );
